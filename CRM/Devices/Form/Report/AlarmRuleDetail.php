...
class CRM_Devices_Form_Report_AlarmRuleDetail extends CRM_Report_Form
{
...
//        'Organization',
    ...
    public function select()
    {
        ...
        $this->_columnHeaders = [];
        $this->_component = [
            $this->_aliases['civicrm_o8_device_alarm_rule'],
        ];
...
//            CRM_Core_Error::debug_var('table', $table);
    ...
    public function from()
    {
      ...
      FROM civicrm_contact {$this->_aliases['civicrm_contact']} {$this->_aclFrom}
        ...
        if (!empty($this->_selectComponent[$civicrm_o8_device_alarm_rule])) {
            $this->_formComponent[$civicrm_o8_device_alarm_rule] = <<<HERESQL
        FROM
        civicrm_contact {$civicrm_contact}
        INNER JOIN civicrm_o8_device_alarm_rule {$civicrm_o8_device_alarm_rule}
          ON {$civicrm_contact}.id = {$civicrm_o8_device_alarm_rule}.contact_id
HERESQL;
        }
    }
...
//        CRM_Core_Error::debug_var('where', $this->_where);
    ...
    public function clauseComponent()
    {
        ...
        foreach ($this->_component as $val) {
//            CRM_Core_Error::debug_var('valcomponent', $val);
...
//
                    ...
                    if ($component == 'o8_device_alarm_rule_civireport') {

                        if ($val = CRM_Utils_Array::value('civicrm_o8_device_alarm_rule_alarm_rule_rule_id', $row)) {
//                            CRM_Core_Error::debug_var('val1', $val);
                            $componentRows[$contactID][$component][$rowNum]['civicrm_o8_device_alarm_rule_alarm_rule_rule_id']
                                = CRM_Core_PseudoConstant::getLabel("CRM_Devices_BAO_AlarmRule", "rule_id", $val);
                        }
                        if ($val = CRM_Utils_Array::value('civicrm_o8_device_alarm_rule_alarm_rule_sensor_id', $row)) {
//                            CRM_Core_Error::debug_var('val2', $val);
                            $componentRows[$contactID][$component][$rowNum]['civicrm_o8_device_alarm_rule_alarm_rule_sensor_id']
