...
class CRM_Devices_Form_Report_DeviceAlertDetail extends CRM_Report_Form
{
...
//  }
    ...
    public function from()
    {
        ...
        $this->_from .= "
      INNER JOIN civicrm_o8_device_data {$this->_aliases['civicrm_o8_device_data']}
        ON {$this->_aliases['civicrm_contact']}.id = {$this->_aliases['civicrm_o8_device_data']}.contact_id
      INNER JOIN civicrm_o8_device_alarm {$this->_aliases['civicrm_o8_device_alarm']}
        ON {$this->_aliases['civicrm_contact']}.id = {$this->_aliases['civicrm_o8_device_alarm']}.contact_id
        AND {$this->_aliases['civicrm_o8_device_data']}.id = {$this->_aliases['civicrm_o8_device_alarm']}.device_data_id
      INNER JOIN civicrm_o8_device_alert {$this->_aliases['civicrm_o8_device_alert']}
        ON {$this->_aliases['civicrm_contact']}.id = {$this->_aliases['civicrm_o8_device_alert']}.contact_id
        AND {$this->_aliases['civicrm_o8_device_alarm']}.id = {$this->_aliases['civicrm_o8_device_alert']}.alarm_id
      INNER JOIN civicrm_o8_device_alert_rule {$this->_aliases['civicrm_o8_device_alert_rule']}
        ON {$this->_aliases['civicrm_contact']}.id = {$this->_aliases['civicrm_o8_device_alert_rule']}.contact_id
        AND {$this->_aliases['civicrm_o8_device_alert_rule']}.id = {$this->_aliases['civicrm_o8_device_alert']}.alert_rule_id
...
//                            CRM_Core_Error::debug_var('val', $val);
                ...
                $rows[$rowNum]['civicrm_o8_device_alert_rule_alert_rule_addressee_id']
                    = $addressee;
            }

            if ($val = CRM_Utils_Array::value('civicrm_o8_device_data_sensor_id', $row)) {
//                            CRM_Core_Error::debug_var('val', $val);
                $val = intval($val);
                $rows[$rowNum]['civicrm_o8_device_data_sensor_id']
                    = CRM_Core_PseudoConstant::getLabel("CRM_Devices_BAO_AlarmRule", "sensor_id", $val);
            }
            if ($val = CRM_Utils_Array::value('civicrm_o8_device_data_date', $row)) {
//                            CRM_Core_Error::debug_var('val', $val);
                $rows[$rowNum]['civicrm_o8_device_data_date']
                    = $val;
            }
            if ($val = CRM_Utils_Array::value('civicrm_o8_device_data_device_id', $row)) {
                $val = intval($val);
//                            CRM_Core_Error::debug_var('val', $val);
                ...
                $rows[$rowNum]['civicrm_o8_device_data_device_id']
                    = $device_code;
            }
            if ($val = CRM_Utils_Array::value('civicrm_o8_device_alarm_alarm_rule_id', $row)) {
                $val = intval($val);
//                            CRM_Core_Error::debug_var('val', $val);
                $rule_sql = "SELECT CONCAT(ovs.label, ' ', ovr.label, ' ', ar.sensor_value) alarm_rule
                           from civicrm_o8_device_alarm_rule ar
                           ...
                           inner join civicrm_option_group ogr on ovr.option_group_id = ogr.id 
                                                                       and ogr.name = 'o8_device_rule_type'
    
    where ar.id = {$val}";
//                CRM_Core_Error::debug_var('rule_sql', $rule_sql);
                $rule_code = CRM_Core_DAO::singleValueQuery($rule_sql);
                $rows[$rowNum]['civicrm_o8_device_alarm_alarm_rule_id']
                    = $rule_code;
            }
            $lastKey = $rowNum;
        }
    ...
    public function sectionTotals()
    {
        ...
        if (!empty($this->_sections)) {
            ...
            if (array_search("civicrm_o8_device_data_total_amount", $this->_selectAliases) !==
                FALSE
            ) {
                $addtotals = ", sum(civicrm_o8_device_data_total_amount) as sumcontribs";
